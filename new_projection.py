...
def shift_coords(fotogramma_n, las_file, t):
    ...
    myradians = math.atan2(float(previous_frame[1]) - float(curret_frame[1]),
                           float(previous_frame[2]) - float(curret_frame[2]))  # boh
...
def find_signal_pos(res,points_las2,segnale,curret_frame):
    points_las = points_las2
    i = segnale
    '''
...
def clustering(point_masked, curret_frame):
    ...
    if (len(df.index) > 0):  # i need to have points inside on the first place
        ...
        if (min < min_distance):  # i consider points not too far
            ...
            if(diff < 1): #consdrro le cluster non troppo "sparse" dato che potrebbero indicare erorri
                print(diff)
...
def remove_outlier(array):
    print(len(array))
    df_in = pd.DataFrame(array)
    df_in["D"] = df_in[0]%10 + df_in[1]%10  + df_in[2]%10
    q1 = df_in["D"].quantile(0.25)
    q3 = df_in["D"].quantile(0.75)
    iqr = q3 - q1  # Interquartile range
    fence_low = q1 - 1.5 * iqr
    fence_high = q3 + 1.5 * iqr
    df_out = df_in.loc[(df_in["D"] > fence_low) & (df_in["D"] < fence_high)]
    print(len(df_out))
    df_out = df_out.drop(['D'], axis = 1)
    ...
    with open('dati_mappa_test2.csv', 'w') as file:
        ...
        for i in signal_list[starting:]:  # QUESTA LISTA PARTE DA 1
            ...
            if (N > min_conf):
                ...
                tal = PointProjection()
                points_las_shift, points_las, curret_frame = shift_coords(fotogramma_n, las_file,
                                                                          t)  # Terzo parametro sono i migliaia
                ...
                for i in range(int(len(position_gps) / 3)):
                    ...
                    name = names[i] #aggiungere il fotogramma
                    name = str(name).replace("[", "")
                    name = str(name).replace("]", "")
                    name = str(name).replace("'", "")
                    confidence = name[-4:].replace(")","")
                    name = name[:-4]
                    name = name.replace(")", "")
